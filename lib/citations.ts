function formatAuthorsAPA(authors: { name: string }[]): string {
  ...
  if (authors.length <= 20) {
    const authorList = authors.slice(0, authors.length - 1).map(a => a.name).join(', ');
    return `${authorList}, & ${authors[authors.length - 1].name}.`;
  }
  const authorList = authors.slice(0, 19).map(a => a.name).join(', ');
  return `${authorList}, ... ${authors[authors.length - 1].name}.`;
}
